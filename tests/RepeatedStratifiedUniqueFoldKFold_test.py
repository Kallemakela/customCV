...
from customCV.stratified import RepeatedStratifiedUniqueFoldKFold
...
def test_stratification_perfect(CVClass):
    CVClass = RepeatedStratifiedUniqueFoldKFold
    n_samples = 40
    n_repeats = 5
    n_splits = 10
    n_classes = 4
    ...
    X = np.arange(n_samples)
    cv = CVClass(n_splits=n_splits, n_repeats=n_repeats, random_state=42, tolerance=0)
...
def test_stratification_uneven(CVClass):
    ...
    y = (np.arange(n_samples) // np.ceil(n_samples / n_classes)).astype(int)
    X = np.arange(n_samples)
    cv = CVClass(n_splits=n_splits, n_repeats=n_repeats, random_state=42, tolerance=1)
    overall_ratios = np.bincount(y) / n_samples
    u, c = np.unique(y, return_counts=True)
    splits = list(cv.split(X, y))
    assert len(u) == n_classes
