...
def test_initialization():
    # Test initialization
    cv = RepeatedUniqueFoldKFold(n_splits=4, n_repeats=3, random_state=42)
...
def test_split_functionality():
    ...
    y = np.ones(n_samples)
    cv = RepeatedUniqueFoldKFold(n_splits=n_splits, n_repeats=n_repeats, random_state=42)
...
def test_unique_folds_across_repeats():
    ...
    X = np.arange(n_samples)
    y = np.ones(n_samples)
    cv = RepeatedUniqueFoldKFold(n_splits=2, n_repeats=3, random_state=42)
...
def test_random_state():
    ...
    X = np.arange(n_samples)
    y = np.ones(n_samples)
    cv_0 = RepeatedUniqueFoldKFold(n_splits=2, n_repeats=3, random_state=0)
    cv_1 = RepeatedUniqueFoldKFold(n_splits=2, n_repeats=3, random_state=1)
...
def test_exhaustion():
    ...
    X = np.arange(n_samples)
    y = np.ones(n_samples)
    cv = RepeatedUniqueFoldKFold(n_splits=n_splits, n_repeats=n_repeats, random_state=None)
...
def test_exhaustion_random():
    ...
    X = np.arange(n_samples)
    y = np.ones(n_samples)
    cv = RepeatedUniqueFoldKFold(n_splits=n_splits, n_repeats=n_repeats, random_state=0)
...
def test_error_handling():
    ...
    with pytest.raises(ValueError):
        cv = RepeatedUniqueFoldKFold(n_splits=5, n_repeats=6)
        list(cv.split(X, y))
    ...
    with pytest.raises(ValueError):
        cv = RepeatedUniqueFoldKFold(n_splits=10, n_repeats=10)  # More splits than samples
        list(cv.split(X, y))
